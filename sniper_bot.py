...
import pytz  # <<--- เพิ่มบรรทัดนี้ลงไปครับเฮีย!
# ================= โซนตั้งค่า (Config) =================
...
async def check_price_loop():
    ...
    # --- แก้เวลาเป็น 21:30 น. (เวลาตลาดเปิด) ---
    tz = pytz.timezone('Asia/Bangkok')
    now = datetime.now(tz)
    
    # ถ้าอยู่ในช่วง 21:30 - 21:35 ให้ล้างประวัติการเตือนของวันก่อนทิ้ง
    if now.hour == 21 and now.minute >= 30 and now.minute < 36:
        alerted_tickers.clear()
        print(f"[{now.strftime('%H:%M:%S')}] 🔔 ตลาดเปิดแล้ว! ล้างประวัติเพื่อเตรียมแจ้งเตือนรอบใหม่")
    # --------------------------------------------------------
